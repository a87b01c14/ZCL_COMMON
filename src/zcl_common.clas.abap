...
public section.
  ...
  types:
    tt_abap_table_keycompdescr TYPE STANDARD TABLE OF abap_table_keycompdescr .
  ...
  class-methods EXPORT_EXCEL
    ...
    raising
      ZCX_EXCEL .
  class-methods EXPORT_EXCEL_SMW0
    importing
  ...
  class-methods OPEN_WEBURL
    importing
      !IV_WEBURL type STRING .
  ...
  class-methods SPLIT_GROUP
    ...
    exceptions
      KEYS_NOT_FOUND .
  class-methods CONVERT_TO_OUTPUT
    changing
      value(CS_INPUT) type ANY .
  class-methods CONVERT_TO_INPUT
    changing
      value(CS_INPUT) type ANY .
  class-methods CONVERT_TO_OUTPUT_V
    importing
      value(IV_INPUT) type ANY
      value(IV_CONVEXIT) type DFIES-CONVEXIT optional
    changing
      value(EV_OUTPUT) type ANY .
  class-methods CONVERT_TO_INPUT_V
    importing
      value(IV_INPUT) type ANY
      value(IV_CONVEXIT) type DFIES-CONVEXIT optional
    changing
      value(EV_OUTPUT) type ANY .
  class-methods CONVERT_TO_OUTPUT_S
    changing
      value(CS_INPUT) type ANY .
  class-methods CONVERT_TO_INPUT_S
    changing
      value(CS_INPUT) type ANY .
  class-methods CONVERT_TO_OUTPUT_T
    changing
      value(CT_INPUT) type ANY TABLE .
  class-methods CONVERT_TO_INPUT_T
    changing
      value(CT_INPUT) type ANY TABLE .
  PROTECTED SECTION.
  PRIVATE SECTION.
...
*       ship_point        = ship_point
  ...
  METHOD post_dn.
    ...
    lt_header_deadlines[] = VALUE #( deliv_numb = iv_vbeln
                                     ...
                                     ( timetype = 'WSHDRWADTI' timestamp_utc = lv_timestamp_utc )"发货时间
...
*       I_MBLNR                   =
  ...
  METHOD print_excel.
    DATA: lo_data            TYPE REF TO zcl_excel_template_data,
          ...
          lo_reader          TYPE REF TO zif_excel_reader,
          lo_worksheet       TYPE REF TO zcl_excel_worksheet,
          lo_worksheet_clone TYPE REF TO zcl_excel_worksheet,
...
*  COPY worksheet
        LOOP AT lt_data ASSIGNING FIELD-SYMBOL(<fs_data>) WHERE clone = abap_true.
          lo_worksheet = lo_excel->get_worksheet_by_name( <fs_data>-sheet ).
          lo_worksheet_clone = lo_excel->clone_worksheet( lo_worksheet ).
          IF <fs_data>-rename IS NOT INITIAL.
            lo_worksheet_clone->set_title( <fs_data>-rename ).
            <fs_data>-sheet = <fs_data>-rename.
          ENDIF.
        ENDLOOP.
        LOOP AT lt_data ASSIGNING <fs_data> WHERE clone = abap_false AND rename IS NOT INITIAL .
          lo_worksheet = lo_excel->get_worksheet_by_name( <fs_data>-sheet ).
          lo_worksheet->set_title( <fs_data>-rename ).
          <fs_data>-sheet = <fs_data>-rename.
        ENDLOOP.
        lo_data = NEW #( it_data = lt_data ).
        ...
        CALL FUNCTION 'ZFUN_DISPLAY_EXCEL'
          EXPORTING
            ...
            iv_autosave  = iv_autosave    " 是否自动保存文件
...
*                                                              └── REF TO IT_DATA LINE
    ...
    IF it_keys IS NOT INITIAL.
      "lt_keys_components = CORRESPONDING #( it_keys MAPPING name = table_line )."sometimes not work
      lt_keys_components = VALUE #( FOR wa_key IN it_keys ( name = wa_key ) ).
    ELSE.
    ...
    IF ct_count[] IS INITIAL.
      ...
      "select count(*) from @ct_data as a group by 不可用
      "为了速度考虑，最好是从外部传值进来
      LOOP AT ct_data ASSIGNING <wa_line>.
        read_table <wa_line> <table_count> <line_count>.
        IF sy-subrc = 0.
        ...
        ELSE.
          ...
          ASSIGN COMPONENT 'GROUP_COUNT' OF STRUCTURE <line_count> TO <wa_value>.
          <wa_value> = 1.
          INSERT <line_count> INTO <table_count> INDEX sy-tabix.
        ENDIF.
      ENDLOOP.
    ELSE.
...
*│                           ├── REF TO IT_DATA LINE
  ...
  METHOD convert_to_output.
    DATA:lo_elem TYPE REF TO cl_abap_elemdescr,
         lo_type TYPE REF TO cl_abap_typedescr.

    FIELD-SYMBOLS <fs_data> TYPE any.

    lo_type ?=  cl_abap_typedescr=>describe_by_data( cs_input ).
    CASE lo_type->type_kind.
      WHEN cl_abap_typedescr=>typekind_dref.
        ASSIGN cs_input->* TO <fs_data>.
        lo_type ?=  cl_abap_typedescr=>describe_by_data( <fs_data> ).
        CASE lo_type->type_kind.
          WHEN cl_abap_typedescr=>typekind_struct1 OR cl_abap_typedescr=>typekind_struct2.
            convert_to_output_s( CHANGING cs_input = cs_input ).
          WHEN OTHERS.
            CHECK cl_abap_datadescr=>is_read_only( cs_input ) = abap_false.
            convert_to_output( CHANGING cs_input = <fs_data> ).
        ENDCASE.
      WHEN cl_abap_typedescr=>typekind_table.
        convert_to_output_t( CHANGING ct_input = cs_input ).
      WHEN cl_abap_typedescr=>typekind_struct1 OR cl_abap_typedescr=>typekind_struct2.
        convert_to_output_s( CHANGING cs_input = cs_input ).
      WHEN OTHERS.
        CHECK lo_type IS INSTANCE OF cl_abap_elemdescr.
        lo_elem ?= lo_type.
        CHECK lo_elem->edit_mask IS NOT INITIAL.
        convert_to_output_v( EXPORTING iv_input = cs_input iv_convexit = lo_elem->edit_mask+2(5) CHANGING ev_output = cs_input ).
    ENDCASE.
  ENDMETHOD.


  METHOD convert_to_input.
    DATA:lo_elem TYPE REF TO cl_abap_elemdescr,
         lo_type TYPE REF TO cl_abap_typedescr.
    ...
    CASE lo_type->type_kind.
      WHEN cl_abap_typedescr=>typekind_dref.
        ...
        CASE lo_type->type_kind.
          WHEN cl_abap_typedescr=>typekind_struct1 OR cl_abap_typedescr=>typekind_struct2.
            convert_to_input_s( CHANGING cs_input = cs_input ).
          WHEN OTHERS.
            CHECK cl_abap_datadescr=>is_read_only( cs_input ) = abap_false.
            convert_to_input( CHANGING cs_input = <fs_data> ).
        ENDCASE.
      WHEN cl_abap_typedescr=>typekind_table.
        convert_to_input_t( CHANGING ct_input = cs_input ).
      WHEN cl_abap_typedescr=>typekind_struct1 OR cl_abap_typedescr=>typekind_struct2.
        convert_to_input_s( CHANGING cs_input = cs_input ).
      WHEN OTHERS.
        CHECK lo_type IS INSTANCE OF cl_abap_elemdescr.
        lo_elem ?= lo_type.
        CHECK lo_elem->edit_mask IS NOT INITIAL.
        convert_to_input_v( EXPORTING iv_input = cs_input iv_convexit = lo_elem->edit_mask+2(5) CHANGING ev_output = cs_input ).
    ENDCASE.
  ENDMETHOD.


  METHOD convert_to_output_v.
    DATA: lo_elem TYPE REF TO cl_abap_elemdescr,
          lo_type TYPE REF TO cl_abap_typedescr.

    DATA: lv_funcname TYPE rs38l-name.
    DATA lv_value_out TYPE rsanu_t_fieldvalue.

    "Set initial output value to input value.
    ev_output = iv_input.
    IF iv_convexit IS INITIAL.
      lo_elem ?= cl_abap_elemdescr=>describe_by_data( iv_input ).

      "If the data has no DDIC structure, exit
      CHECK lo_elem->is_ddic_type( ) = abap_true.

      DATA(ls_dfies) = lo_elem->get_ddic_field( sy-langu ).

      "If DDIC structure has no conversion exit, exit.
      CHECK ls_dfies-convexit IS NOT INITIAL.
    ...
    IF iv_convexit = 'ALPHA'.
      ...
      RETURN.
    ENDIF.
    lv_funcname = |CONVERSION_EXIT_{ iv_convexit }_OUTPUT|.
    ...
    IF sy-subrc = 0 .
      CALL FUNCTION lv_funcname
        ...
        IMPORTING
          output = lv_value_out.
      ev_output =  lv_value_out.
    ENDIF.
  ENDMETHOD.


  METHOD convert_to_input_v.
    DATA: lo_elem TYPE REF TO cl_abap_elemdescr,
          lo_type TYPE REF TO cl_abap_typedescr.
    ...
    lv_funcname = |CONVERSION_EXIT_{ iv_convexit }_INPUT|.
  ...
  METHOD convert_to_output_s.
    ...
    CASE lo_type->type_kind.
      ...
      WHEN OTHERS.
        ASSIGN cs_input TO <fs_data>.
    ENDCASE.
    lo_struct ?=  cl_abap_typedescr=>describe_by_data( <fs_data> ).
    lt_fields = cl_salv_data_descr=>read_structdescr( lo_struct ).
    LOOP AT lt_fields INTO ls_fields.
      ASSIGN COMPONENT ls_fields-fieldname OF STRUCTURE <fs_data> TO <fs_value>.
      CASE ls_fields-inttype.
        WHEN cl_abap_typedescr=>typekind_dref.
          ...
          ENDCASE.
        WHEN cl_abap_typedescr=>typekind_table.
          convert_to_output_t( CHANGING ct_input = <fs_value> ).
        WHEN OTHERS.
          CHECK <fs_value> IS ASSIGNED.
          CHECK ls_fields-convexit IS NOT INITIAL.
          CHECK cl_abap_datadescr=>is_read_only( REF #( <fs_value> ) ) = abap_false.
          convert_to_output_v( EXPORTING iv_input = <fs_value> iv_convexit = ls_fields-convexit CHANGING ev_output = <fs_value> ).
      ENDCASE.
    ENDLOOP.
  ENDMETHOD.


  METHOD convert_to_input_s.
    DATA:lo_type   TYPE REF TO cl_abap_typedescr,
         lo_struct TYPE REF TO cl_abap_structdescr,
         lo_elem   TYPE REF TO cl_abap_elemdescr.
    DATA lt_fields TYPE ddfields.
    ...
    LOOP AT lt_fields INTO ls_fields.
      ...
      CASE ls_fields-inttype.
        WHEN cl_abap_typedescr=>typekind_dref.
          ...
          CASE lo_type->type_kind.
            WHEN cl_abap_typedescr=>typekind_struct1 OR cl_abap_typedescr=>typekind_struct2.
              convert_to_input_s( CHANGING cs_input = <fs_data> ).
            WHEN OTHERS.
              CHECK cl_abap_datadescr=>is_read_only( cs_input ) = abap_false.
              convert_to_input( CHANGING cs_input = <fs_data> ).
          ENDCASE.
        WHEN cl_abap_typedescr=>typekind_table.
          convert_to_input_t( CHANGING ct_input = <fs_value> ).
        WHEN OTHERS.
          CHECK <fs_value> IS ASSIGNED.
          CHECK ls_fields-convexit IS NOT INITIAL.
          CHECK cl_abap_datadescr=>is_read_only( REF #( <fs_value> ) ) = abap_false.
          convert_to_input_v( EXPORTING iv_input = <fs_value> iv_convexit = ls_fields-convexit CHANGING ev_output = <fs_value> ).
      ENDCASE.
    ENDLOOP.
  ENDMETHOD.


  METHOD convert_to_output_t.
    DATA lo_data TYPE REF TO data.
    DATA lo_type  TYPE REF TO cl_abap_typedescr.
    FIELD-SYMBOLS <fs_input> TYPE any.
    LOOP AT ct_input ASSIGNING <fs_input>.
      lo_type ?= cl_abap_typedescr=>describe_by_data( <fs_input> ).
      CASE lo_type->type_kind.
        WHEN cl_abap_typedescr=>typekind_dref.
          convert_to_input_s( CHANGING cs_input = <fs_input> ).
        WHEN OTHERS.
          lo_data = REF #( <fs_input> ).
          "convert_to_output_s( CHANGING cs_input = <fs_input> ) dump if any field read only
          convert_to_output_s( CHANGING cs_input = lo_data ).
      ENDCASE.
    ENDLOOP.
  ENDMETHOD.


  METHOD convert_to_input_t.
    DATA lo_data TYPE REF TO data.
    DATA lo_type  TYPE REF TO cl_abap_typedescr.
    FIELD-SYMBOLS <fs_input> TYPE any.
    LOOP AT ct_input ASSIGNING <fs_input>.
      ...
      CASE lo_type->type_kind.
        ...
        WHEN OTHERS.
          lo_data = REF #( <fs_input> ).
          "convert_to_input_s( CHANGING cs_input = <fs_input> ) dump if any field read only
          convert_to_input_s( CHANGING cs_input = lo_data ).
      ENDCASE.
    ENDLOOP.
  ENDMETHOD.


  METHOD export_excel_smw0.
    DATA: lo_data            TYPE REF TO zcl_excel_template_data,
          lo_excel           TYPE REF TO zcl_excel,
          lo_reader          TYPE REF TO zif_excel_reader,
          lo_worksheet       TYPE REF TO zcl_excel_worksheet,
...
*  COPY worksheet
        ...
        "export
        CREATE OBJECT lo_writer TYPE zcl_excel_writer_2007.
        xdata = lo_writer->write_file( lo_excel ).
        CLEAR lt_file_content.
        cl_scp_change_db=>xstr_to_xtab(
          EXPORTING
            im_xstring = xdata
        ...
        IF iv_filename IS NOT INITIAL.
          lv_filename = |{ iv_filename }{ sy-datum }{ sy-uzeit }.XLSX|.
        ELSE.
          lv_filename = |Export{ sy-datum }{ sy-uzeit }.XLSX|.
        ENDIF.
*
        lv_fullpath = save_file_dialog( iv_filename = CONV #( lv_filename ) ).
