...
CLASS lc_test_runtime DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      test_simple_std_key          FOR TESTING,
...
CLASS lc_test_runtime IMPLEMENTATION.
  ...
  METHOD init_data.
    ...
    DATA(lo_random) = cl_abap_random=>create( ). " 创建随机数生成器实例
    DO 100 TIMES.
      ls_data = VALUE #( id = sy-index
      group = lo_random->intinrange( low = 1 high = 20 )
      value = lo_random->intinrange( low = 1 high = 100 ) ).
